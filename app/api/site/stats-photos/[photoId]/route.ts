...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    if (typeof imageUrl === "string" && imageUrl.startsWith("https://")) {
      const site = await setStatsPhotoUrl(photoId, imageUrl, version);
    ...
    const site = await uploadStatsPhoto(photoId, file, version);
    const record = await readSiteRecord();
    revalidatePublicContent();
    return NextResponse.json(site, {
      headers: siteResponseHeaders(record.version),
...
export async function DELETE(request: Request, context: RouteContext) {
  try {
    const { photoId } = await context.params;
    const version = expectedSiteVersion(request);
    const site = await clearStatsPhoto(photoId, version);
    const record = await readSiteRecord();
    revalidatePublicContent();
    return NextResponse.json(site, {
      headers: siteResponseHeaders(record.version),
